...
export class Player {
    ...
    constructor(cosmeticsConfig = null) {
        ...
        this.cosmetics = {
            shipStrokeColor: '#161310',
            boostRGB: '184, 50, 63',
            particleColor: 'rgba(184, 50, 63, 0.8)',
            shipDesignIndex: 1,
            bulletColor: '#161310'
        };
    ...
    draw(ctx, gameTime, mouseVelocity, isShipMode, scale = 1, morph = 1, bank = 0) {
        ...
        if (isShipMode && thrust > 3) {
            ...
            ctx.closePath();
            const boostRGB = this.cosmetics?.boostRGB || '184, 50, 63';
        ...
        if (this.powerUpState.shield.active) {
            ...
            ctx.strokeStyle = `rgba(184, 50, 63, ${0.35 + Math.sin(gameTime * 0.005) * 0.2})`;
            ctx.lineWidth = 2;
            ctx.stroke();
        }
    ...
    shoot(gameTime, isShooting, mouseVelocity) {
        ...
        if (now - this.lastShotTime > cooldown) {
            ...
            const createBullet = (angleOffset = 0) => {
                ...
                const bulletColor = this.cosmetics && this.cosmetics.bulletColor ? this.cosmetics.bulletColor : '#161310';
